 ...
 * - Each chart lives inside its own card with an accent swatch
 * - Uses CSS vars for colors so it integrates with other pages
 * - Replace the Map placeholder with your Graph.jsx (Leaflet) component when ready
 */
...
const pointsToPath = (pts) => pts.map((p, i) => `${i === 0 ? "M" : "L"} ${p[0]} ${p[1]}`).join(" ");
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      ...
      {data.map((v, i) => {
        ...
        const y = h - pad - barH;
        return <rect key={i} x={x} y={y} rx="4" width={bw} height={barH} fill="url(#barG2)" />;
      })}
    </svg>
  );
}
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {values.map((v, i) => {
        ...
        return <path key={i} d={path} fill={colors[i % colors.length]} stroke="var(--card)" strokeWidth="0.5" />;
      })}
      {donut && <circle cx={size / 2} cy={size / 2} r={inner} fill="var(--card)" />}
      {donut && <text x="50%" y="50%" textAnchor="middle" dy="6" fontSize="13" fill="var(--text)" fontWeight="700">{Math.round((values[0] / total) * 100)}%</text>}
    </svg>
  );
}
...
function RadarChart({ axes = [], values = [], size = 180 }) {
  ...
  const valuePts = values.map((v, i) => polarToCartesian(cx, cy, (v / max) * r, i * step));
  return (
    <svg width="100%" viewBox={`0 0 ${size} ${size}`}>
      {axisPts.map((p, i) => <line key={i} x1={cx} y1={cy} x2={p[0]} y2={p[1]} stroke="var(--border)" strokeWidth="0.8" />)}
      <polygon points={axisPts.map(p => p.join(",")).join(" ")} fill="transparent" stroke="var(--border)" strokeWidth="0.6" />
      <polygon points={valuePts.map(p => p.join(",")).join(" ")} fill="var(--accent)" fillOpacity="0.14" stroke="var(--accent)" strokeWidth="1.2" />
      {axes.map((a, i) => {
        const label = polarToCartesian(cx, cy, r + 14, i * step);
        return <text key={i} x={label[0]} y={label[1]} fontSize="11" fill="var(--subtext)" textAnchor="middle" dominantBaseline="middle">{a}</text>;
      })}
    </svg>
  );
}
...
export default function Dashboard() {
  ...
  const bubbleData = useMemo(() => [
    { x: 20, y: 40, r: 12 }, { x: 40, y: 70, r: 18 }, { x: 60, y: 30, r: 9 },
    { x: 80, y: 85, r: 22 }, { x: 100, y: 50, r: 14 }
  ], []);
  ...
  // accents
  const paletteColors = ["#0ea5e9", "#06b6d4", "#7c3aed", "#34d399", "#fb7185", "#f59e0b"];
  const accents = {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-7 space-y-6">
            {/* Line & Area Section */}
            <section className="rounded-3xl border" style={{ borderColor: "var(--border)", background: "var(--card)", boxShadow: theme === "dark" ? "0 6px 30px rgba(0,0,0,0.4)" : "0 6px 20px rgba(15,23,42,0.04)" }}>
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-lg" style={{ background: accents.line }} />
                  <div>
                    <div style={{ color: "var(--text)", fontWeight: 700 }}>Line & Area</div>
                    <div className="text-xs" style={{ color: "var(--subtext)" }}>Trends & infographics</div>
                  </div>
                </div>
                <div className="text-xs" style={{ color: "var(--subtext)" }}>Volume</div>
              </div>

              <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-xl p-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                  <div className="text-sm font-semibold mb-2" style={{ color: "var(--text)" }}>Weekly Traffic</div>
                  <LineChart data={weeklyTraffic} />
                </div>
                <div className="rounded-xl p-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                  <div className="text-sm font-semibold mb-2" style={{ color: "var(--text)" }}>Infographic (Area)</div>
            ...
            {/* Pie / Donut Section */}
            <section className="rounded-3xl border" style={{ borderColor: "var(--border)", background: "var(--card)", boxShadow: theme === "dark" ? "0 6px 30px rgba(0,0,0,0.45)" : "0 6px 20px rgba(15,23,42,0.03)" }}>
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-lg" style={{ background: accents.pie }} />
                  <div>
                    <div style={{ color: "var(--text)", fontWeight: 700 }}>Pie & Donut</div>
                    <div className="text-xs" style={{ color: "var(--subtext)" }}>Category distribution</div>
                  </div>
                </div>
              ...
              <div className="p-4 flex items-center gap-4">
                <div style={{ width: 140 }}>
                  <PieChart values={categoryDistribution} colors={paletteColors} size={120} donut inner={30} />
                </div>
                <div className="flex-1">
                  {["Health", "Politics", "Scam", "Adult", "Spam", "Other"].map((label, i) => {
                    const total = categoryDistribution.reduce((a, b) => a + b, 0) || 1;
                    const pct = Math.round((categoryDistribution[i] / total) * 100) || 0;
                    return (
                      <div key={i} className="flex items-center justify-between py-1">
                        <div className="flex items-center gap-3">
                          <span className="w-3 h-3 rounded-sm" style={{ background: paletteColors[i % paletteColors.length] }} />
            ...
            {/* Bar & Mixed Section */}
            <section className="rounded-3xl border" style={{ borderColor: "var(--border)", background: "var(--card)", boxShadow: theme === "dark" ? "0 10px 40px rgba(0,0,0,0.5)" : "0 6px 20px rgba(15,23,42,0.03)" }}>
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-lg" style={{ background: accents.bar }} />
                  <div>
                    <div style={{ color: "var(--text)", fontWeight: 700 }}>Bar & Mixed</div>
                    <div className="text-xs" style={{ color: "var(--subtext)" }}>Volume & overlay metrics</div>
                  </div>
                </div>
                <div className="text-xs" style={{ color: "var(--subtext)" }}>Monthly</div>
              </div>

              <div className="p-4">
                <div className="mb-4">
                  <div className="text-sm font-semibold mb-2" style={{ color: "var(--text)" }}>Monthly — Volume & Credibility</div>
                  <div style={{ width: "100%", height: 220 }}>
                    <svg width="100%" viewBox="0 0 700 220" preserveAspectRatio="xMidYMid meet">
                      <defs>
                      ...
                      {mixedBar.map((v, i) => {
                        ...
                        return <rect key={i} x={x} y={y} width={bw} height={barH} rx="3" fill="url(#mbar2)" />;
                      })}
                      {/* overlay line */}
                      {(() => {
                        const w = 700, pad = 40, h = 160;
                        const pts = mixedLine.map((v, i) => {
                          const x = pad + i * ((w - pad * 2) / (mixedLine.length - 1));
                          const y = 220 - pad - (v / Math.max(...mixedLine)) * h;
                          return [x, y];
                        });
                        return <path d={pointsToPath(pts)} fill="none" stroke="var(--accent)" strokeWidth="3" strokeLinecap="round" />;
                      })()}
                    </svg>
                  </div>
                </div>
            ...
            <section className="rounded-3xl border" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-lg" style={{ background: accents.radar }} />
                  <div>
                    <div style={{ color: "var(--text)", fontWeight: 700 }}>Radar & Polar</div>
                    <div className="text-xs" style={{ color: "var(--subtext)" }}>Multi-axis & radial</div>
                  </div>
                </div>
                  ...
                  <svg width="100%" viewBox="0 0 200 200">
                    <g transform="translate(100,100)">
                      {polarValues.map((v, i) => {
                        ...
                        return <path key={i} d={p} fill={paletteColors[i % paletteColors.length]} stroke="var(--card)" strokeWidth="0.6" />;
                      })}
                    </g>
                  </svg>
                </div>
            ...
            <section className="rounded-3xl border" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-lg" style={{ background: accents.scatter }} />
                  <div>
                    <div style={{ color: "var(--text)", fontWeight: 700 }}>Scatter & Bubble</div>
                ...
                <div className="rounded-xl p-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                  ...
                  <svg width="100%" viewBox="0 0 400 160">
                    ...
                    {scatterData.map((s, i) => {
                      ...
                      return <circle key={i} cx={x} cy={y} r={3 + (i % 3)} fill="var(--accent)" fillOpacity="0.9" />;
                    })}
                  </svg>
                </div>
                ...
                <div className="rounded-xl p-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                  ...
                  <svg width="100%" viewBox="0 0 220 140">
                    ...
                    {bubbleData.map((b, i) => {
                      ...
                      return <circle key={i} cx={x} cy={y} r={b.r} fill={paletteColors[i % paletteColors.length]} fillOpacity="0.92" />;
                    })}
                  </svg>
                </div>
              </div>
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6">
            {/* Map card */}
            <section className="rounded-3xl border overflow-hidden" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
              <div className="px-4 py-3 flex items-center gap-3 border-b" style={{ borderColor: "var(--border)" }}>
                <span className="w-10 h-10 rounded-lg" style={{ background: accents.map }} />
                <div>
                  <div style={{ color: "var(--text)", fontWeight: 700 }}>Geographic View</div>
                  <div className="text-xs" style={{ color: "var(--subtext)" }}>Interactive map of reported items</div>
                </div>
              </div>

              <div className="w-full h-96 relative">
                {/* Replace placeholder with your Graph.jsx */}
                {/* <Graph className="absolute inset-0 w-full h-full" /> */}
                {/* <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="inline-block px-4 py-2 rounded-xl mb-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                      <div style={{ color: "var(--text)", fontWeight: 700 }}>Map placeholder</div>
                      <div style={{ color: "var(--subtext)", fontSize: 12 }}>Insert Graph.jsx (Leaflet) here</div>
                    </div>
                    <div className="text-xs" style={{ color: "var(--subtext)" }}>
                      Map will show severity markers. Click a marker to open a report.
                    </div>
                  </div>
                </div> */}
                <Graph />
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                <div className="rounded-xl p-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                  ...
                  <div className="mt-2 font-semibold" style={{ color: "var(--text)" }}>Mumbai • Delhi • Bengaluru</div>
                </div>
                <div className="rounded-xl p-3" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)" }}>
                  <div className="text-xs" style={{ color: "var(--subtext)" }}>Avg Response Time</div>
                  <div className="mt-2 font-semibold" style={{ color: "var(--text)" }}>2.4 hrs</div>
                </div>
              </div>
            </section>

            {/* Sources & credibility */}
            <section className="rounded-3xl border p-4" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
              <div className="flex items-center justify-between mb-3">
                <div style={{ color: "var(--text)", fontWeight: 700 }}>Sources & Credibility</div>
                    ...
                    <div key={i} className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div style={{ color: "var(--text)", fontWeight: 600 }}>Source {i + 1}</div>
                          <div style={{ color: "var(--subtext)" }}>{pct}%</div>
                        </div>
                        <div className="h-2 w-full rounded-full overflow-hidden mt-2" style={{ background: "var(--border)" }}>
                          <div style={{ width: `${pct}%`, height: "100%", background: `linear-gradient(90deg, ${accents.map})` }} />
                        </div>
                      </div>
                    </div>
                  );
            ...
            <section className="rounded-3xl border p-4" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
              <div className="flex items-center justify-between mb-2">
                <div style={{ color: "var(--text)", fontWeight: 700 }}>Quick Actions</div>
                <div className="text-xs" style={{ color: "var(--subtext)" }}>Policy tools</div>
              </div>
              <div className="flex flex-col gap-3">
                <button className="px-4 py-2 rounded-xl text-left" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)", border: "1px solid var(--border)", color: "var(--text)" }}>Bulk review flagged (120)</button>
                <button className="px-4 py-2 rounded-xl text-left" style={{ background: theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(15,23,42,0.02)", border: "1px solid var(--border)", color: "var(--text)" }}>Export suspicious content</button>
                <button className="px-4 py-2 rounded-xl text-left" style={{ background: "var(--accent)", color: "white" }}>Open case manager</button>
              </div>
            </section>
          </div>
        </div>
